...
function setupHero(board, player_color)
    ...
    figure.setRotation({hrot.x ,hrot.y+180, hrot.z})
    if (player_color == "Teal") or (player_color == "Green") or (player_color == "Blue") then
        teamColor = {0, 0, 1}
    elseif (player_color == "Red") or (player_color == "Orange") or (player_color == "Yellow") then
        teamColor = {1, 0, 0}
    else
        teamColor = nil
    end
    figure.setColorTint(teamColor)

    -- Tokens setup
    local tokens = getObjectFromGUID(tokenGUIDs[hero])
    if tokens ~= nil then
        tokens.setPosition({hpos.x-(7*hforward.z),1,hpos.z+(12*hforward.z)})
        tokens.setRotation({hrot.x ,hrot.y+180, hrot.z})
    end
    ...
    broadcastToAll(player_color .. " chose " .. hero, player_color)
end
...
function minionPush()
    ...
    -- Winning via push trackers running out (timeout)
    if pushCounter < pushCopies then
        ...
        if pushState == 1 then
            broadcastToAll("(No push trackers left.)", {0, 0, 1})
            broadcastToAll("Blue Team wins via Timeout!", {0, 0, 1})

        elseif pushState == -1 then
            broadcastToAll("(No push trackers left.)", {1, 0, 0})
            broadcastToAll("Red Team wins via Timeout!", {1, 0, 0})
    ...
    -- Winning via push
    elseif pushState > 1 then
        broadcastToAll("Blue Team wins by pushing into Red Team's base!", {0, 0, 1})
        pushState = 1
    elseif pushState < -1 then
        broadcastToAll("Red Team wins by pushing into Blue Team's base!", {1, 0, 0})
        pushState = -1
    end
...
function undoPush(mainBoard)
    ...
    -- sets pushState back to itself prior to the previous push
    pushState = pushState - prevPushDiff
    -- sets pushCounter back by 1 and return counter to ptStack
    pushCounter = pushCounter - 1
